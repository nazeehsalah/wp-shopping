...
/**
 * Generated class for the ProductIndetailsPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-product-indetails',
  templateUrl: 'product-indetails.html',
})
export class ProductIndetailsPage {
  product: any;
  woo: any;
  reviews: any[] = [];
  selectedOptions: any = {};
  ...
  async check(justSelectedAttribute) {
    ...
    if (count_ != count) {
      ...
      loading.present();
      this.productVariations = JSON.parse((await this.woo.getAsync('products/' + this.product.id + '/variations/')).body);
